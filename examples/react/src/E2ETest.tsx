...
const E2ETest = () => {
  ...
  const init = async () => {
    ...
    // try {
    //     await walletManager.connect('mock1')

    //     const chain = walletManager.chains.find(c => c.chainName === 'osmosis')

    //     if (!chain) {
    //         return
    //     }

    //     const mock1Wallet = walletManager.wallets.find(w => w.option?.name === 'mock1')

    //     if (!mock1Wallet) {
    //         return
    //     }

    //     const clientFactory = await walletManager.createClientFactory(mock1Wallet, 'osmosis')


    //     const client = await clientFactory.getSigningClient()

    //     const address = await mock1Wallet.getAccount(chain.chainId)

    //     console.log(address.address)

    //     await creditFromStarship('http://localhost:8007/credit', address.address, 'uosmo')

    //     const balance = await client.getBalance(address.address, 'uosmo')

...
import { starshipConfig } from './starship-config';
...
export const useChain = (chainName: string) => {
    ...
    const config = starshipConfig as ChainConfig;
    ...
    const creditFromFaucet = async (
        ...
        const faucetEndpoint = `http://localhost:${config.chains.find((chain) => chain.id === chainID)!.ports.faucet
            }/credit`;
        if (!denom) {

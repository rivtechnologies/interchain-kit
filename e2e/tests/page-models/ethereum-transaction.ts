...
export class EthereumTransactionPage {
  readonly page: Page;
  ...
  // 导航到以太坊交易页面
  async goto() {
    await this.page.goto('/ethereum/transaction');
  }
  ...
  // 获取发送者钱包区块
  getSenderWallet() {
    return new EthereumSenderWallet(this.page);
  }

  // 获取接收者钱包区块
  getReceiverWallet() {
    return new EthereumReceiverWallet(this.page);
  }
}

class EthereumSenderWallet {
  readonly page: Page;
  ...
  // 发送 Sepolia ETH
  async sendSepoliaETH() {
    await this.rootLocator.getByRole('button', { name: 'send Sepolia ETH' }).click();
  }
